function OUT = DLV_HN_milkdata_v53(cd_txt,FN_HA,FN_HALI,FN_HADD,FN_HN,FN_HN2,cd_H,temp_dir)
...
% Voluntary HNDiagnose
hnd_H = readtable([cd_H FN_HN '_headers.txt'],'ReadVariableNames',0);    % read variable names
hnd_H = hnd_H{:,:}';                          % convert to cell array and transpose
writecell(hnd_H,[temp_dir 'FN_HN.txt'],'Delimiter',';');  % write headernames to file
system(['copy "' temp_dir 'FN_HN.txt"+' '"' cd FN_HN '.txt" "'  temp_dir 'FN_HN.txt"']);  % combine files using system cmd
fid  = fopen([temp_dir 'FN_HN.txt'],'r'); f=fread(fid,'*char')'; fclose(fid);
f=f(1:length(f)-1);
fid  = fopen([temp_dir 'FN_HN.txt'],'w'); fwrite(fid,f);fclose(fid);
...
FNS = {FN_BA,FN_ALS,FN_SMY,FN_VMY,FN_HN};
for i = 1:length(FNS)           % length
    FN{i} = [cd FNS{i} ext{1}]; % all three
end
clear i j FN_BA FN_ALS FN_SMY FN_VMY FNS cd ext FN_SMY FN_VMY FN_HN
...
opts = setvartype(opts,{'OID','HNBiometricMeasurement','DaysSincePreviousMastitisAlarm','DiagnoseType','Risk','Alarm','Reliability','Trend','ChronicMastitisIndex','Class','QuarterInfo'},'double');
opts = setvartype(opts,{'DateAndTime'},'datetime');
opts.SelectedVariableNames = {'OID','HNBiometricMeasurement','DateAndTime','DiagnoseType','Risk','Alarm','Reliability','Trend','ChronicMastitisIndex','Class','QuarterInfo'};
e = readtable(FN{5},opts);               % HN
...
col_HN = {'OID','HNBiometricMeasurement','DateAndTime','DiagnoseType','Risk','Alarm','Reliability','Trend','ChronicMastitisIndex','Class','QuarterInfo'};
...
idx_HN = zeros(1,length(col_HN));         % to fill in - column indices
...
for i = 1:length(col_HN)
    idx_HN(i) = find(contains(e.Properties.VariableNames,col_HN{i})==1); 
end
clear col_BA col_ALS col_SMY col_VMY col_HN i
...
e.Properties.VariableNames = {'OID3','HNBiometricMeasurement','date','DiagnoseType','Risk','Alarm','Reliability','Trend','ChronicMastitisIndex','Class','QuarterInfo'};

clear idx_ALS idx_BA idx_AHD idx_VMY idx_SMY idx_HN
...
% % % %  HOW TO MERGE HN DATA INTO THIS FILE?????
